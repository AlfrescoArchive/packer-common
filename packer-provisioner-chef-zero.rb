Racker::Processor.register_template do |t|
  t.provisioners = {
    ...
    10 => {
      "chef" => {
        "execute_command"=> "cat /tmp/packer-chef-client/first-boot.json && cat /tmp/packer-chef-client/client.rb && ls -l /etc/chef/cookbooks && ls -l /etc/chef/databags && sudo chef-client --no-color --local-mode -c /tmp/packer-chef-client/client.rb -j /tmp/packer-chef-client/first-boot.json -l {{user `chef_log_level`}}",
